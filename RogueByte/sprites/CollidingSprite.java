...
public class CollidingSprite implements DisplayableSprite {
    ...
    public CollidingSprite(double centerX, double centerY) {
        ...
        if (image == null) {
            try {
                image = ImageIO.read(new File("res/Byte.png"));
            } catch (IOException e) {
     ...
     *
     * The second type of collision detection is more precise but also requires more processing. The Byte sprite is
    ...
    public void update(Universe universe, KeyboardInput keyboard, long actual_delta_time) {
        ...
        boolean collidingWithByte = checkCollisionWithByte(universe.getSprites(), deltaX, deltaY);
        ...
        //only move if there is no collision with Byte in any dimension and no collision with barrier in X dimension
        if ((collidingWithByte == false) && (collidingBarrierX == false)) {
            this.centerX += deltaX;
        }
        //only move if there is no collision with Byte in any dimension and no collision with barrier in Y dimension
        if ((collidingWithByte == false) && (collidingBarrierY == false)) {
            this.centerY += deltaY;
        }
    }
    ...
    private boolean checkCollisionWithByte(ArrayList<DisplayableSprite> sprites, double deltaX, double deltaY) {

        //deltaX and deltaY represent the potential change in position
        boolean colliding = false;

        for (DisplayableSprite sprite : sprites) {
            if (sprite instanceof ByteSprite) {
